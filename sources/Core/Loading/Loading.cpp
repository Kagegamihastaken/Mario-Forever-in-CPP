...
namespace MFCPP {
	...
	std::vector<uint8_t> IO::Loadbyte(const std::filesystem::path &fileName)
	{
	...
	void IO::LoadTexture(sf::Texture& texture, const std::filesystem::path &path, const sf::IntRect& rect) {
	...
	std::vector<uint8_t> IO::GetFileDataInByte(const std::filesystem::path &path) {
	...
	void IO::LoadImageFile(sf::Image& image, const std::filesystem::path &path) {
		//std::vector<uint8_t> vec = Loadbyte(path);
