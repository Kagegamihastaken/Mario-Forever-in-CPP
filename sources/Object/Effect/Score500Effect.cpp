...
Score500Effect::Score500Effect(EffectManager &manager, const sf::Vector2f &position) : Effect(manager) {
...
void Score500Effect::setPreviousData() {
    if (isDestroyed()) return;

    setPreviousPosition(getCurrentPosition());
}

void Score500Effect::interpolateData(float alpha) {
    if (isDestroyed()) return;
