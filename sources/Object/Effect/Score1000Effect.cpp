...
Score1000Effect::Score1000Effect(EffectManager &manager, const sf::Vector2f &position) : Effect(manager) {
    setCurrentPosition(position);
    setPreviousPosition(position);
    setInterpolatedPosition(position);
    setOrigin(sf::Vector2f(17.f, 15.f));
    setHitbox(sf::FloatRect({0.f, 0.f}, {34.f, 16.f}));
...
void Score1000Effect::setPreviousData() {
    if (isDestroyed()) return;

    setPreviousPosition(getCurrentPosition());
}

void Score1000Effect::interpolateData(float alpha) {
    if (isDestroyed()) return;
