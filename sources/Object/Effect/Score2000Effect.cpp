...
Score2000Effect::Score2000Effect(EffectManager &manager, const sf::Vector2f &position)
    : Effect(manager),
    m_transform(position, sf::Vector2f(17.f, 15.f), sf::degrees(0.f)){
    m_hitbox = sf::FloatRect({0.f, 0.f}, {35.f, 16.f});
...
void Score2000Effect::updatePreviousData() {
...
sf::Vector2f Score2000Effect::getPosition() {
    return m_transform.getCurrentPosition();
}

sf::Vector2f Score2000Effect::getOrigin() {
    return m_transform.getOrigin();
}

sf::FloatRect Score2000Effect::getHitbox() {
    return m_hitbox;
}

bool Score2000Effect::isDestroyed() {
    return m_transform.isDestroyed();
}

void Score2000Effect::draw(float alpha) {
...
void Score2000Effect::teleport(sf::Vector2f val) {
