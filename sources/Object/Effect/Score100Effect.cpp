...
Score100Effect::Score100Effect(EffectManager &manager, const sf::Vector2f &position) : Effect(manager) {
    setCurrentPosition(position);
    setPreviousPosition(position);
    setInterpolatedPosition(position);
    setOrigin(sf::Vector2f(12.f, 15.f));
    setHitbox(sf::FloatRect({0.f, 0.f}, {26.f, 16.f}));
...
void Score100Effect::setPreviousData() {
    if (isDestroyed()) return;

    setPreviousPosition(getCurrentPosition());
}

void Score100Effect::interpolateData(float alpha) {
    if (isDestroyed()) return;
