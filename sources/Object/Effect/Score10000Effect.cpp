...
Score10000Effect::Score10000Effect(EffectManager &manager, const sf::Vector2f &position) : Effect(manager) {
    setCurrentPosition(position);
    setPreviousPosition(position);
    setInterpolatedPosition(position);
    setOrigin(sf::Vector2f(21.f, 15.f));
    setHitbox(sf::FloatRect({0.f, 0.f}, {42.f, 16.f}));
...
void Score10000Effect::setPreviousData() {
    if (isDestroyed()) return;

    setPreviousPosition(getCurrentPosition());
}

void Score10000Effect::interpolateData(float alpha) {
    if (isDestroyed()) return;
