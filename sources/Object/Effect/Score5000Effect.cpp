...
Score5000Effect::Score5000Effect(EffectManager &manager, const sf::Vector2f &position) : Effect(manager) {
    setCurrentPosition(position);
    setPreviousPosition(position);
    setInterpolatedPosition(position);
    setOrigin(sf::Vector2f(18.f, 15.f));
    setHitbox(sf::FloatRect({0.f, 0.f}, {36.f, 16.f}));
...
void Score5000Effect::setPreviousData() {
    if (isDestroyed()) return;

    setPreviousPosition(getCurrentPosition());
}

void Score5000Effect::interpolateData(float alpha) {
    if (isDestroyed()) return;
