...
GreenPiranhaUp::GreenPiranhaUp(EnemyManager &manager, const sf::Vector2f &position) : Enemy(manager) {
...
void GreenPiranhaUp::setPreviousData() {
    if (isDestroyed() || isDisabled()) return;
    setPreviousPosition(getCurrentPosition());
}

void GreenPiranhaUp::interpolateData(float alpha) {
    if (isDestroyed() || isDisabled()) return;
