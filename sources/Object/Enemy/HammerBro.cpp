...
void HammerBro::updatePreviousData() {
...
void HammerBro::statusUpdate(float deltaTime) {
    ...
    if (m_state == 0) {
        if (Scroll::isOutOfScreenYBottom(MFCPP::CollisionObject(m_transform.getCurrentPosition(), getOrigin(), getHitbox()), 0.f))
            Destroy();
        if (!Scroll::isOutOfScreen(MFCPP::CollisionObject(m_transform.getCurrentPosition(), getOrigin(), getHitbox()), 0.f))
            if (isDisabled()) {
                setDisabled(false);
                return;
            }
        ...
        if (Scroll::isOutOfScreenYBottom(MFCPP::CollisionObject(m_transform.getCurrentPosition(), getOrigin(), getHitbox()), 0.f)) Destroy();
    }
...
void HammerBro::draw(float alpha) {
...
sf::Vector2f HammerBro::getPosition() {
    return m_transform.getCurrentPosition();
}

sf::Vector2f HammerBro::getOrigin() {
    return m_transform.getOrigin();
}

sf::FloatRect HammerBro::getHitbox() {
    return m_hitbox;
}

bool HammerBro::isDestroyed() {
