...
BulletBillLauncherFlipped::BulletBillLauncherFlipped(CustomTileManager &manager, const sf::Vector2f &position) : CustomTile(manager) {
...
void BulletBillLauncherFlipped::setPreviousData() {
    if (isDestroyed()) return;
    setPreviousPosition(getCurrentPosition());
}

void BulletBillLauncherFlipped::interpolateData(float alpha) {
    if (isDestroyed()) return;
