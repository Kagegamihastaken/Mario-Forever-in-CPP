...
LuckyBlockBeetroot::LuckyBlockBeetroot(CustomTileManager &manager, const sf::Vector2f &position) : CustomTile(manager) {
...
void LuckyBlockBeetroot::setPreviousData() {
    if (isDestroyed()) return;
    setPreviousPosition(getCurrentPosition());
}

void LuckyBlockBeetroot::interpolateData(float alpha) {
    if (isDestroyed()) return;
...
void LuckyBlockBeetroot::Hit() {
    ...
    if (Mario::getPowerState() == 0)
        ...
        GameScene::enemyManager.addEnemy<Beetroot>(getCurrentPosition() + sf::Vector2f(16.f, 0.f));
