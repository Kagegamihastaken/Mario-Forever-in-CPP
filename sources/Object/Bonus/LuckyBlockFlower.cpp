...
LuckyBlockFlower::LuckyBlockFlower(CustomTileManager &manager, const sf::Vector2f &position) : CustomTile(manager) {
...
void LuckyBlockFlower::setPreviousData() {
    if (isDestroyed()) return;
    setPreviousPosition(getCurrentPosition());
}

void LuckyBlockFlower::interpolateData(float alpha) {
    if (isDestroyed()) return;
